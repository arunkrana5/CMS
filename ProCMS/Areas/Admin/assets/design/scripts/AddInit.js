...
$("#anchorgenerateURL").bind("click", function () {
    GenerateURL();
});
...
function GenerateURL() {
    ...
    var URLinputCheckFor = $("#URLinputCheckFor").val();
    var URLinputContentID = $("#URLinputContentID").val();
   
        $.ajax({
            url: "/CommonAjax/GenerateURLJSon",
            type: "Get",
            data: { ID: URLinputContentID, Value: URLinput, Type: URLinputCheckFor },
			success: function (data) {
				if (!data.Status) {
					...
					$(".hdnURLResponse").val(data.AdditionalMessage);
					$("#URLinput").val(data.AdditionalMessage);
                }
            },
            error: function (er) {
                alert(er);
